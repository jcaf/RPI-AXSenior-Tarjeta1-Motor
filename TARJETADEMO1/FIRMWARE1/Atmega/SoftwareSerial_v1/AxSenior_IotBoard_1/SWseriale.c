...
uint8_t SWseriale_read()
{
    ...
    if (SWseriale_recv_buffer_last_pro_item == SWSERIALE_RECV_BUF_SIZE)
        SWseriale_recv_buffer_last_pro_item=0; // rollover
...
bool SWseriale_write(uint8_t* data_array, uint8_t data_size)
{
    ...
    // Entry conditions
    if ((data_size == 0) || (data_size > SWSERIALE_SEND_BUF_SIZE))
        return false; // size to send is not acceptable
    ...
    for (uint8_t i=0; i<data_size; i++)
    {
        ...
        if (SWseriale_send_buffer_last_added == SWSERIALE_SEND_BUF_SIZE)
            SWseriale_send_buffer_last_added = 0; // rollover
    }
...
void SWseriale_listen(SWseriale_mode_enum listening_mode)
{
    ...
    if ((listening_mode != IDLE_MODE) && (listening_mode != RECV_MODE))
        listening_mode = IDLE_MODE; // Standard value (not possible to have, for example, SEND_MODE)
...
bool SWseriale_prepareToSend()
{
    ...
    // Entry conditions
    if (SWseriale_send_buffer_to_send_now == SWseriale_send_buffer_last_added)
        return false; // no bytes required to be sent
    if ((!FORCE_SEND) && (SWseriale_mode != IDLE_MODE))
        return false; // Serial bus is not in IDLE condition (it is either in SEND or RECV), so it is not possible to send due to busy condition
    ...
    if (SWseriale_send_buffer_to_send_now == SWSERIALE_SEND_BUF_SIZE)
        SWseriale_send_buffer_to_send_now = 0; // rollover
...
ISR(TIMER2_COMPA_vect)
{
    ...
    if (SWseriale_mode == RECV_MODE)
    {
        ...
        else if (recv_bit_num == 9)  // when all 10 bits (0-9) have been received (all byte is completed), byte has to be added to the buffer
        {
            ...
            if (SWseriale_recv_buffer_last_log_item == SWSERIALE_RECV_BUF_SIZE)
                SWseriale_recv_buffer_last_log_item=0; // rollover
        ...
        else if (recv_bit_num == 9+BITS_WAITING_AFTER_RECV)  // the bus is kept in "RECV" state for a little bit more, to allow the next byte to arrive
        {
            ...
            if (!SWseriale_prepareToSend())
                SWseriale_listen(IDLE_MODE); // Checks if any byte has to be sent (this is necessary in case any byte to be sent is pending the end of reception state). If not, timer is stopped.
            return;
        }
        recv_bit_num++;
    }
